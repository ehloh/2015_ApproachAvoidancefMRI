function [ V ] = bp01(x, modelinput)
% [ V ] = bp01(x, modelinput)
%   Value function (same as b01)
%
%  - Inputs
...
% Scale epsilon for binary
epsilonbinary=epsilon*(3/2);   % TrinomEpsilon/ (1/3)  should be proportional to BinomEpsilon/(1/2)
% epsilonbinary=0;
...
%       (posVChoice, V Accept/Reject after Exploring), including epsilon (irreducible stochasticity)
...
posVChoice(:,:,1)=ppActBomb.* repmat(fPar.cF_FL, [1 nT_cF]) + (1-ppActBomb).* repmat(data_cF(:,col.NTokens)', [nPP 1]);
...
pAccGivNoBombSeen=repmat(epsilonbinary, [1 nT_cF]) +  (1-2.*repmat(epsilonbinary, [1 nT_cF])).*pAccGivNoBombSeen;   % apply epsilon (scaled for binary)
% pRejGivNoBombSeen=exp((repmat(beta, [1 nT_cF]).*posVChoice(:,:,2)))   ./   (   exp(repmat(beta, [1 nT_cF]).*posVChoice(:,:,1))  +   exp(repmat(beta, [1 nT_cF]).*posVChoice(:,:,2))    );
% pRejGivNoBombSeen=repmat(epsilonbinary, [1 nT_cF]) +  (1-2.*repmat(epsilonbinary, [1 nT_cF])).*pRejGivNoBombSeen;   % apply epsilon (scaled for binary)
pRejGivNoBombSeen=1-pAccGivNoBombSeen;
...
V_AccGivNoSee=    ppActBomb  .* repmat(fPar.cF_FL,[1 nT_cF]) +            (1-ppActBomb ) .* repmat(data_cF(:,col.NTokens)', [nPP 1]);
EV_NoSee= pAccGivNoBombSeen .*  V_AccGivNoSee  + pRejGivNoBombSeen .* 0;
...
posVChoice(:,:,1)= repmat(data_ct(:, col.NTokens)', [nPP 1])  .* (1-ppActBomb);
posVChoice(:,:,2)= repmat(data_ct(:, col.NTokens)', [nPP 1])  .* ppActBomb;
...
pAccGivNoBombSeen=repmat(epsilonbinary, [1 nT_ct]) +  (1-2.*repmat(epsilonbinary, [1 nT_ct])).*pAccGivNoBombSeen;   % apply epsilon (scaled for binary)
pRejGivNoBombSeen=1-pAccGivNoBombSeen;
% pRejGivNoBombSeen=exp((repmat(beta, [1 nT_ct]).*posVChoice(:,:,2)))   ./   (   exp(repmat(beta, [1 nT_ct]).*posVChoice(:,:,1))  +   exp(repmat(beta, [1 nT_ct]).*posVChoice(:,:,2))    );
% pRejGivNoBombSeen=repmat(epsilonbinary, [1 nT_ct]) +  (1-2.*repmat(epsilonbinary, [1 nT_ct])).*pRejGivNoBombSeen;   % apply epsilon (scaled for binary)

% [3] V(Explore) =   p(See)*V(See)     +     p(~See)* V(~See)    +   Exploration Cost
EVGivenNotSee =pAccGivNoBombSeen.*posVChoice(:,:,1)+pRejGivNoBombSeen.*posVChoice(:,:,2);
