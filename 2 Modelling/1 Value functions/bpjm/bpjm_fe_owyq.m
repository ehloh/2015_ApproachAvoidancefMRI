...
fPar.cF_FL=x(:,5);
ExploreBonusFixed=x(:,6);
ExploreBonusVar_cf=x(:,7);
ExploreBonusVar_ct=x(:,8);
...
[ V ] = bpjm01(x, {modname data, fPar,col});
...
% Add variable exploration bonus (across tasks)
nTrials=size(data,1); nPP=size(x,1); 
V(:,:,3)=V(:,:,3)  +   repmat(ExploreBonusFixed, [1 nTrials]);
binomvar=data(:, col.pLoss).*(1-data(:, col.pLoss));

% Add variable exploration bonus to cF 
nTrials_exp=find(data(:, col.Task)==1);  
V(:, nTrials_exp, 3)=V(:, nTrials_exp, 3)  +   repmat( ExploreBonusVar_cf, [1 length( nTrials_exp )]).*repmat(  data(nTrials_exp,  col.EntropyNTok)', [nPP 1]);

% Add variable exploration bonus to ct
nTrials_ct=find(data(:, col.Task)==2);  
V(:, nTrials_ct, 3)=V(:, nTrials_ct, 3)  +   repmat( ExploreBonusVar_ct, [1 length( nTrials_ct )]).*repmat(   binomvar(nTrials_ct)', [nPP 1]);
