...
DistortVaccExp=40./(1+exp(-x(:,2)))  - 20;  % -20 < i < 20 
% Omega=x(:,3);       % Omega: Multiplier-utility of variance 

Omega= (1./(2+2.*exp(-x(:,3)))) - 0.25; 
...
FixedLoss=repmat(fPar.cF_FL,[nPP nT_cF]); 
ExploreCost=repmat(fPar.cF_EC,[nPP nT_cF]); 
V_cF=nan(nPP, nT_cF ,3);
pActBomb=repmat(data_cF(:,col.pLoss)',[nPP 1]);   % Task-space variables
NTok=repmat(data_cF(:,col.NTokens)',[nPP 1]);
EnvThreat=repmat(data_cF(:,col.EnvThreat)',[nPP 1]);
beta=repmat(b, [1 nT_cF]);    % Free parameters
DistortVaccExp=repmat(dve, [1 nT_cF]) ;
om=repmat(Omega, [1 nT_cF]);    

% Theoretical Outcome variance/std dev and value according to mean-variance theory (D'acremont & Bossaerts 2008 CABN)
% See fpar_conflict for details 
OutcomeMean=repmat(data_cF(:,col.EV)',[nPP 1]);  
StanDev = (1-pActBomb).*(NTok- OutcomeMean).^2+ pActBomb.*(FixedLoss-OutcomeMean).^2; 

% [1] V(Accept) & V(Reject)
V_cF(:,:,1)= pActBomb .* FixedLoss +   (1-pActBomb).*NTok   + StanDev.*om;
V_cF(:,:,2)=0;
...
posVacc=ppActBomb.* FixedLoss + (1-ppActBomb).* NTok;
posVacc=posVacc+DistortVaccExp + StanDev.*om;
%
posVrej=0;
...
clearvars om StanDev OutcomeMean EV_NoSee EnvThreat ExploreCost FixedLoss NTok V_AccGivNoSee data_cF nT_cF pAccGivNoBombSeen pActBomb posVacc posVrej ppActBomb ppActBomb 
if size(whos,1)>13; disp('Workspace clear from cF to ct: more parameters in workspace than usual'); end
...
ExploreCost=repmat(fPar.ct_EC,[nPP nT_ct]); 
V_ct=nan(nPP, nT_ct,3);
pActBomb=repmat(data_ct(:,col.pLoss)',[nPP 1]);   % Task-space variables
NTok=repmat(data_ct(:,col.NTokens)',[nPP 1]);
EnvThreat=repmat(data_ct(:,col.EnvThreat)',[nPP 1]);
beta=repmat(b, [1 nT_ct]);    % Free parameters
DistortVaccExp=repmat(dve, [1 nT_ct]) ;
...
% [2b] Likelihood of Accepting, if one Explores and sees no Bomb
posVacc= (1-ppActBomb) .* NTok;
posVacc=posVacc+DistortVaccExp;
%
posVrej= ppActBomb .* NTok;
pAccGivNoBombSeen=posVacc>posVrej;
