...
% Model's value function: [ V ] = bpm01(x, modelinput)
%   Value function with inoptimal posterior probability calculation (x Multiplier)
%
%  - Inputs
...
if sum(isinf(UnOptim(:)))~=0; disp('Infinite UnOptim posterior probability distortion!'); end
...
%ppABscaled=repmat(UnOptim, [1 nT_cF]).*ppActBomb;
% ppABscaled=1 ./ 1+ exp(-repmat(UnOptim, [1 nT_cF]).*ppActBomb);   % Sigmoid constraint, 0<x <1
...
pAccGivNoBombSeen=repmat(epsilonbinary, [1 nT_cF]) +  (1-2.*repmat(epsilonbinary, [1 nT_cF])).*pAccGivNoBombSeen;   % epsilon 
...
ppActBomb=repmat(ppActBomb', [nPP 1]);
%ppABscaled=repmat(UnOptim, [1 nT_ct]).*ppActBomb;
% ppABscaled=1 ./ 1+ exp(-repmat(UnOptim, [1 nT_ct]).*ppActBomb);   % Sigmoid constraint, 0<x <1
...
pAccGivNoBombSeen=repmat(epsilonbinary, [1 nT_ct]) +  (1-2.*repmat(epsilonbinary, [1 nT_ct])).*pAccGivNoBombSeen;   % epsilon 
