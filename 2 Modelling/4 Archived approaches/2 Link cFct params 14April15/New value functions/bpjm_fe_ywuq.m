...
V(:, nTrials_exp, 3)=V(:, nTrials_exp, 3)  +   repmat(  ExploreBonusVar_cf, [1 length( nTrials_exp )]).*  repmat(  binomvar(nTrials_exp)'  , [nPP 1]);

% Add variable exploration bonus to ct
nTrials_ct=find(data(:, col.Task)==2);  
V(:, nTrials_ct, 3)=V(:, nTrials_ct, 3)  +   repmat(  ExploreBonusVar_ct, [1 length( nTrials_ct )]).*repmat(data(nTrials_ct,  col.Entropy)', [nPP 1]);
