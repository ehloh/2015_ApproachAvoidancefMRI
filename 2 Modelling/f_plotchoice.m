function [b] = f_plotchoice(p,data, PosOf1st, axissettings,titles)
% Generate heat-plots for subject behaviour (individual/group mean)
% [b] = f_plotchoice(p,data, PosOf1st, title, axissettings)
%
%         p:                      settings, including for task set up
%         data:                 raw subject data
%         PosOf1st:          Subplot position of 1st plot 
%                                     (format as per subplot template)
%         axissettings:      1=On, 0=Off
%         titles:                 1=On, 0=Off (Accept/Reject/Explore)
%
...
%         subjname='p18_YD';
%         label=subjname; PosOf1st=[2 3 1]; axissettings=0; titles=1;
%         stagenum=2;
%         stage='gooddata';
%         where='H:\5 Conflict Exploration\3 Analysis\1 Data\Good data';
%         %
%         load([subjname filesep subjname '_file_gooddata.mat'])
%         eval(['data=' stage '.data;'])
%         eval(['p=' stage '.settings;'])
%
% -------------------------------------------------------------------------

for o1=1:1 % Documentation for 'data'
...
%     Col 15:     Outcome in amount (nTokens)
end
...
% Accept -------------
subplot(PosOf1st(1), PosOf1st(2),PosOf1st(3))
imagesc(b.accept,[0 1])
axis('square')
if titles==1
    title('% Accept')
    if isempty(p.tasktype)==0 && p.tasktype==2
        title('% NoBomb')
    end
end
if axissettings==1
...
% Reject -------------
subplot(PosOf1st(1), PosOf1st(2),PosOf1st(3)+1)
...
% Explore -------------
subplot(PosOf1st(1), PosOf1st(2),PosOf1st(3)+2) 
...
for o1=1:1 % Other behavioural variables (disused) ------------------    
