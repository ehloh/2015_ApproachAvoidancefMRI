...
error('This model is problematic because ct only has positive PEs. Fails at the stage of FL contrasts. Only p01 has been run. Not ok because using cF trials alone probably doesn''t give enough power');
...
[variables c] = i_ExploreInfoVals( datastruc{1}(datastruc{1}(:,col.TrialValid)==1, :), 'cF_', {'Presented'; 'ExploreInfoPEsign'}, variables, col, c );
[variables c] = i_ExploreInfoVals( datastruc{2}(datastruc{2}(:,col.TrialValid)==1, :), 'ct_', {'Presented'; 'ExploreInfoPEsign'}, variables, col, c );
