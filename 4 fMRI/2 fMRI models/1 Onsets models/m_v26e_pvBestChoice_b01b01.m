...
% softmax-v(Best) regressors (cF/ct trials, with pmods describing values)
[variables c] =  i_Tasktrial_pmod_Competing( datastruc{1}(  datastruc{1}(:, col.TrialValid)==1, :) , variables, 'cF_', {'pvBest';}, col, c );
[variables c] =  i_Tasktrial_pmod_Competing( datastruc{2}(  datastruc{2}(:, col.TrialValid)==1, :), variables, 'ct_', {'pvBest'}, col, c ); 

% % EXPLORE information regressors
[variables c] = i_ExploreInfoVals( datastruc{1}(datastruc{1}(:,col.TrialValid)==1, :), 'cF_', {'Presented';}, variables, col, c );
[variables c] = i_ExploreInfoVals( datastruc{2}(datastruc{1}(:,col.TrialValid)==1, :), 'ct_', {'Presented'; }, variables, col, c );
