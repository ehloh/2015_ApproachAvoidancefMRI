function [Con log] = t1_RLvars(where, log)
% [Con log] = t1_RLvars(where, log)
% Set up 1st-level contrasts for all RL variables.
%   RL variables:   
%         - VExplore (X) 
%         - Entropy (U)
%         - EV (V)
...
% but spec+est model already controls for choice. 
%
...
RLvariables_same={'VExplore'; 'Entropy'; 'pLoss'; 'EnvThreat'; 'NTokens'; 'EntropyNTok'}; % Specified the same across both tasks
RLvariables_diff={'EV'};
...
if  log.execute_contrasts==1
    ...
    col.VExplore=5;
    col.EV=6;
    col.OutcomeMagnitude=9;
    col.OutcomeMean=10;
    col.OutcomeVariance=11;
    col.TrialType_EnvThreat=7;  % New columns, pertaining to 'parvals', for reading Trial-type name. Cols 1-10 are used in instruction-building below.
    col.TrialType_NTokens=8;
    col.EntropyNTok=12;
    tasks={'cF'; 'ct'};
    ...
    % Set up parameters + mean centre
    [ parvals] = fpar_conflict( pars, col);
    parvals=[parvals(:,1:6)-repmat(mean(parvals(:,1:6)), size(parvals,1),1) parvals(:,[col.TrialType_EnvThreat col.TrialType_NTokens])];
    openvar parvals
    ...
    col
    keyboard
    [ parvals_ct] = fpar_control( pars, col); % Control-task parvals - only used for EV
    parvals_ct=[parvals_ct(:,1:6)-repmat(mean(parvals_ct(:,1:6)), size(parvals_ct,1),1) parvals_ct(:,[col.TrialType_EnvThreat col.TrialType_NTokens])];
    ...
    for p=1:length(instruc.singlepar) % Combined  
        ...
        %
        wp.par=[num2cell(parvals) cell(6*6,2)]; j=size(wp.par,2);
    ...
    for t=1:2 % Task comparisons  
        for p=1:length(instruc.singlepar)
            ...
            wp.par=[num2cell(parvals) cell(6*6,4)]; j=size(wp.par,2);
    ...
    for p=1:length(instruc.splitpar)  % Combined  
        ...
        wp.par_cF=[num2cell(parvals) cell(6*6,2)]; j=size(wp.par_cF,2);
    ...
    for t=1:2 % Single task  
        for p=1:length(instruc.splitpar)
            ...
            %
            switch t
                case 1;  wp.par=[num2cell(parvals) cell(6*6,2)]; j=size(wp.par,2);
