...
DONT NEED THIS MODEL. Or use it to generate contrasts for all the RL variables. 
But no need to compare across 2 different variables
...
if  log.execute_contrasts==1
    ...
    error('Stopped here. See text & instructions in function')
    If this is an ok approach, need to tweak instructions so that a given regressor 
    isn''t weighted more than once, by summing the 2 weights. 
    Need to verify: Is this ok, it assumes pLoss and VExplore are in the same currency 
