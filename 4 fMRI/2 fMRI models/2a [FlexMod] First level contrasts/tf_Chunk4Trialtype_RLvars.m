function [Con log] = tf_Chunk4Trialtype_RLvars(where, log)
% Set up 1st-level contrasts for all RL variables. For use with Chunk4Trialtype
% models (i.e. reduced to 3x3 rather than 6x6). Assumes regressors are labelled e.g. cF_t1-1
%
%   RL variables:   
%         - EntropyNTok (O)
%         - VExplore (V) 
...
if  log.execute_contrasts==1
    ...
    col.TrialTypeOrig_EnvThreat=1;  % Columns marking EnvThreat and NTokens levels (for reading Trial-type name). Other columns are for building instructions (weights)
    col.TrialTypeOrig_NTokens=2;
    col.TrialTypeChunk_EnvThreat=3; % chunk cells
    col.TrialTypeChunk_NTokens=4;
    col.EnvThreat=5;
    col.NTokens=6;
    col.pLoss=7;
    col.Entropy=8;
    ...
    pars(:,col.TrialTypeOrig_EnvThreat)=pars(:,col.EnvThreat);
    pars(:,col.TrialTypeOrig_NTokens)=pars(:,col.NTokens)/2;
    ...
    % Chunk: 3x3   % i.e. Cell 1-1: t1-1, t1-2, t2-1, t2-2
    chunk{1,1}= {[1 1]   [1 2]   [2 1]   [2 2]}; chunk{1,2}= {[1 3]   [1 4]   [2 3]   [2 4]}; chunk{1,3}= {[1 5]   [1 6]   [2 5]   [2 6]};
    chunk{2,1}= {[3 1]   [3 2]   [4 1]   [4 2]}; chunk{2,2}= {[3 3]   [3 4]   [4 3]   [4 4]}; chunk{2,3}= {[3 5]   [3 6]   [4 5]   [4 6]};
    chunk{3,1}= {[5 1]   [5 2]   [6 1]   [6 2]}; chunk{3,2}= {[5 3]   [5 4]   [6 3]   [6 4]}; chunk{3,3}= {[5 5]   [5 6]   [6 5]   [6 6]};
    for ee=1:3 % Mark chunk cell (new e and n values)
        for nn=1:3
            for c=1:4 
                parvals_cF(parvals_cF(:, col.TrialTypeOrig_EnvThreat)==chunk{ee,nn}{c}(1) & parvals_cF(:, col.TrialTypeOrig_NTokens)==chunk{ee,nn}{c}(2),  col.TrialTypeChunk_EnvThreat)=ee;
                parvals_cF(parvals_cF(:, col.TrialTypeOrig_EnvThreat)==chunk{ee,nn}{c}(1) & parvals_cF(:, col.TrialTypeOrig_NTokens)==chunk{ee,nn}{c}(2), col.TrialTypeChunk_NTokens)=nn;
                
                parvals_ct(parvals_ct(:, col.TrialTypeOrig_EnvThreat)==chunk{ee,nn}{c}(1) & parvals_ct(:, col.TrialTypeOrig_NTokens)==chunk{ee,nn}{c}(2),  col.TrialTypeChunk_EnvThreat)=ee;
                parvals_ct(parvals_ct(:, col.TrialTypeOrig_EnvThreat)==chunk{ee,nn}{c}(1) & parvals_ct(:, col.TrialTypeOrig_NTokens)==chunk{ee,nn}{c}(2), col.TrialTypeChunk_NTokens)=nn;
            end
        end
    end
    for ee=1:3 % Mark new parameters (mean of constituent cells, assumed Col 5 onwards)
        for nn=1:3
            wc= parvals_cF(parvals_cF(:, col.TrialTypeChunk_EnvThreat)==ee & parvals_cF(:, col.TrialTypeChunk_NTokens)==nn, :);
            parvals_cF(parvals_cF(:, col.TrialTypeChunk_EnvThreat)==ee & parvals_cF(:, col.TrialTypeChunk_NTokens)==nn, 5:end)=repmat(mean(wc(:, 5:end)),4,1);
            
            wc= parvals_ct(parvals_ct(:, col.TrialTypeChunk_EnvThreat)==ee & parvals_ct(:, col.TrialTypeChunk_NTokens)==nn, :);
            parvals_ct(parvals_ct(:, col.TrialTypeChunk_EnvThreat)==ee & parvals_ct(:, col.TrialTypeChunk_NTokens)==nn, 5:end)=repmat(mean(wc(:, 5:end)),4,1);
        end
    end
    parvals_cF=sortrows(parvals_cF, [col.TrialTypeChunk_EnvThreat col.TrialTypeChunk_NTokens]);
    parvals_ct=sortrows(parvals_ct, [col.TrialTypeChunk_EnvThreat col.TrialTypeChunk_NTokens]);
    parvals_cF=[nan(3*3, 2) unique(parvals_cF(:,3:end), 'rows')];
    parvals_ct=[nan(3*3, 2) unique(parvals_ct(:,3:end), 'rows')];
    
    % Mean centre parameters
    parvals_cF=[parvals_cF(:, 1:4)  parvals_cF(:, 5:end)- repmat(mean(parvals_cF(:, 5:end)), 9, 1)];
    parvals_ct=[parvals_ct(:, 1:4)  parvals_ct(:, 5:end)- repmat(mean(parvals_ct(:, 5:end)), 9, 1)];
    ...
    for p=1:length(instruc.singlepar) % Combined  
        ...
        %
        wp.par=[num2cell(parvals_cF) cell(3*3,2)]; j=size(wp.par,2);
        wp.par(:,j)=cellfun(@(x,y)[{['cF_e' num2str(x) '-n' num2str(y)] ; ['ct_e' num2str(x) '-n' num2str(y)]}], wp.par(:,col.TrialTypeChunk_EnvThreat), wp.par(:,col.TrialTypeChunk_NTokens),'UniformOutput', 0);
    ...
    for t=1:2 % Single task
        for p=1:length(instruc.singlepar)
            ...
            %
            wp.par=[num2cell(parvals_cF) cell(3*3,2)]; j=size(wp.par,2);
            wp.par(:,j)=cellfun(@(x,y)[{[tasks{t} '_e' num2str(x) '-n' num2str(y)]}], wp.par(:,col.TrialTypeChunk_EnvThreat), wp.par(:,col.TrialTypeChunk_NTokens), 'UniformOutput', 0);
            eval(['wp.par(:,j-1)=wp.par(:, col.'  instruc.singlepar{p,1} ');'])
            %
            SearchCriteria{k,2}=wp.par(:,j-1:j); SearchCriteria{k,3}=p;
            %
    ...
    for t=1:2 % Task comparisons  
        for p=1:length(instruc.singlepar)
            ...
            wp.par=[num2cell(parvals_cF) cell(3*3,4)]; j=size(wp.par,2);
            
            % Positive task
            wp.par(:,j-2)=cellfun(@(x,y)[{[tasks{t} '_e' num2str(x) '-n' num2str(y)]}], wp.par(:,col.TrialTypeChunk_EnvThreat), wp.par(:,col.TrialTypeChunk_NTokens), 'UniformOutput', 0);
            eval(['wp.par(:,j-3)=wp.par(:, col.'  instruc.singlepar{p,1} ');'])
            
            % Negative task
            wp.par(:,j)=cellfun(@(x,y)[{[tasks{3-t} '_e' num2str(x) '-n' num2str(y)]}], wp.par(:,col.TrialTypeChunk_EnvThreat), wp.par(:,col.TrialTypeChunk_NTokens), 'UniformOutput', 0);
    ...
    for p=1:length(instruc.splitpar)  % Combined  
        ...
        wp.par_cF=[num2cell(parvals_cF) cell(3*3,2)]; j=size(wp.par_cF,2);
        wp.par_cF(:,j)=cellfun(@(x,y)[{['cF_e' num2str(x) '-n' num2str(y)]}], wp.par_cF(:,col.TrialTypeChunk_EnvThreat), wp.par_cF(:,col.TrialTypeChunk_NTokens),'UniformOutput', 0);
        eval(['wp.par_cF(:,j-1)=wp.par_cF(:, col.'  instruc.splitpar{p,1} ');'])
        
        % Control task parameters
        wp.par_ct=[num2cell(parvals_ct) cell(3*3,2)]; jj=size(wp.par_ct,2);
        wp.par_ct(:,jj)=cellfun(@(x,y){['ct_e' num2str(x) '-n' num2str(y)]}, wp.par_ct(:,col.TrialTypeChunk_EnvThreat), wp.par_ct(:,col.TrialTypeChunk_NTokens),'UniformOutput', 0);
    ...
    for t=1:2 % Single task  
        for p=1:length(instruc.splitpar)
            ...
            %
            switch t
                case 1;  wp.par=[num2cell(parvals_cF) cell(3*3,2)]; j=size(wp.par,2);
                case 2;  wp.par=[num2cell(parvals_ct) cell(3*3,2)]; j=size(wp.par,2);
            end
            %
            wp.par(:,j)=cellfun(@(x,y)[{[tasks{t} '_e' num2str(x) '-n' num2str(y)]}], wp.par(:,col.TrialTypeChunk_EnvThreat), wp.par(:,col.TrialTypeChunk_NTokens), 'UniformOutput', 0);
    ...
    for t=1:2 %  Task comparisons  
        for p=1:length(instruc.splitpar)
            ...
            switch t
                case 1;  
                    wp.par1=[num2cell(parvals_cF) cell(3*3,2)]; j=size(wp.par1,2);
                    wp.par2=[num2cell(parvals_ct) cell(3*3,2)]; jj=size(wp.par2,2);
                case 2;  
                    wp.par1=[num2cell(parvals_ct) cell(3*3,2)]; j=size(wp.par1,2);
                    wp.par2=[num2cell(parvals_cF) cell(3*3,2)]; jj=size(wp.par2,2);
            end
            
            % Positive task
            wp.par1(:,j)=cellfun(@(x,y)[{[tasks{t} '_e' num2str(x) '-n' num2str(y)]}], wp.par1(:,col.TrialTypeChunk_EnvThreat), wp.par1(:,col.TrialTypeChunk_NTokens), 'UniformOutput', 0);
            eval(['wp.par1(:,j-1)=wp.par1(:, col.'  instruc.splitpar{p,1} ');'])
            
            % Negative task
            wp.par2(:,jj)=cellfun(@(x,y)[{[tasks{3-t} '_e' num2str(x) '-n' num2str(y)]}], wp.par2(:,col.TrialTypeChunk_EnvThreat), wp.par2(:,col.TrialTypeChunk_NTokens), 'UniformOutput', 0);
