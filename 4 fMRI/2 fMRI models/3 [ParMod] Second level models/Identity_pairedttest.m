...
InstrucChoice=[];
c1=cell(size(RLvariables,1),5);% Compile instructions for RL variables
for i=1:size(RLvariables,1)
    c1{i,1}=[RLvariables{i,1}];  % Conflict-Control
    c1{i,2}=['cF_' RLvariables{i,1}];
    c1{i,3}=['ct_' RLvariables{i,1}];
end
Instruc=vertcat(InstrucChoice, c1);
...
w=load([where.data_brain filesep subjectlist{1} filesep '2 First level' log.FirstLevelThread filesep firstlevelmodel ' Contrasted' filesep 'SPM.mat']);spm=w.SPM;  
...
for a=1:size(Instruc,1)
        ...
        testfolder=[where.resultsfolder filesep 'par ' Instruc{a,1}];
...
for  a=1:size(Instruc,1)
    ...
    wc=load([where.resultsfolder filesep 'par ' Instruc{a,1} filesep 'SPM.mat']);
    if size(wc.SPM.xX.X,2)==2+length(subjectlist)
        wc.subcovars=ones(1,length(subjectlist))*(1)/length(subjectlist);
    else
        wc.subcovars=[];
    end
        
    % Specify the contrast (only + and - available)
    matlabbatch{1}.spm.stats.con.spmmat = {[where.resultsfolder filesep 'par ' Instruc{a,1} filesep 'SPM.mat']};
    ...
    % (1) Overall
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.name=[Instruc{a,1} '_pos'];
    ...
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.name=[Instruc{a,1} '_neg'];
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.convec = [-1 -1 wc.subcovars*-2];
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.sessrep = 'none'; c=c+1; 
    
    % (2) Task-specific
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.name=['cF_' Instruc{a,1} '_pos']; % Conflict
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.convec = [1 0 wc.subcovars*1];
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.sessrep = 'none'; c=c+1; 
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.name=['cF_' Instruc{a,1} '_neg'];
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.convec = [-1 0 wc.subcovars*-1];
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.sessrep = 'none'; c=c+1; 
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.name=['ct_' Instruc{a,1} '_pos']; % Control
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.convec = [0 1 wc.subcovars*1];
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.sessrep = 'none'; c=c+1; 
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.name=['ct_' Instruc{a,1} '_neg'];
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.convec = [0 -1 wc.subcovars*-1];
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.sessrep = 'none'; c=c+1; 
    
    % (3) Difference beteen tasks 
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.name =  ['cF-ct_' Instruc{a,1}]; % Conflict > Control
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.convec = [1 -1];
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.sessrep = 'none'; c=c+1; 
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.name =  ['ct-cF_' Instruc{a,1}]; % Control > Conflict
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.convec = [-1 1];
    matlabbatch{1}.spm.stats.con.consess{c}.tcon.sessrep = 'none';c=c+1; 
