...
%     d_plot={ms_cf ms_ct  ms_cfmct}; % Mean significant from 0
    ...
    %
    k=1; f.FontSize=25;  f.subplotcols=4;  f.subplot_VerHorz=[0.005 0.03]; f.fig_BotTop=[0.001 0.025]; f.fig_LeftRight=[0.1 0.1];  f.nancol=[0 0 0];
...
for o=1:1 % Compile task space design 
    ...
    desvars={'EnvThreat';'NTokens';'pLoss';'Entropy';'EV'; 'EnvThreatOrig'; 'PavConflict'};
    ...
    dcf(:, col.PavConflict) = dcf(:, col.pLoss) .* dcf(:, col.NTokens); dct(:, col.PavConflict)=0;
...
%     cfev=cfee.d_etwarpv; ctev=ctee.d_etwarpv;
    ...
    for s=1:log.n_subjs
        if request.DesignCanon
            ...
            for v=1:length(desvars)
                if strcmp(desvars{v}, 'PavConflict')==0
        ...
        d_design{s,1}(:, col.PavConflict) = d_design{s,1}(:, col.pLoss) .* d_design{s,1}(:, col.NTokens); 
        d_design{s,2}(:, col.PavConflict)=0;
...
% ivs={'Subject'; 'EnvThreat';'Entropy';'EV'; 'PavConflict'};
...
%     [b2 bint rr2 rint stats2]=regress(rr,  [ones(size(d_both,1),1)   d_both(:, col.PavConflict)]); stats2
