...
request.beh_correlations=1;  % Manualy specify in whichever section
%
...
for o1=1:1 % General settings and specifications 
    ...
    else where.where='D:\Dropbox\SCRIPPS\5 Explore fMRI'; where.experiment_folder='C:\Users\eloh\Desktop\2 [Explore]';   where.modfol='D:\Dropbox\SCRIPPS\4 Explore experiment\3 Analysis\4 Fit computational models'; where.data_brain='E:\1 Explore fMRI'; addpath(where.where);  where.modellingdata='C:\Users\eloh\Dropbox\SCRIPPS\4 Explore experiment\3 Analysis\4 Fit computational models\2 Analysis inputs';   end;  addpath(where.where); addpath(where.modfol)
...
if request.do_glm_stepwise
    ...
    allivs={'EnvThreat'; 'NTokens'; 'pLoss';  'Entropy'; 'EV'; 'PavConflict';  'EVConflict';}; request.psig=0.05;
    ...
    for r=1:log.n_rois % Run 
        for t=1:2
            ...
            mdl = GeneralizedLinearModel.stepwise(wr.x, d_betas{r, t+1}(:, col.b), 'linear', 'Criterion', 'bic',  'CategoricalVars',  length(allivs)+1:  length(allivs)+log.n_subjs, 'PredictorVars', 1:length(allivs)) ;
            r_swglm{t}{r,2}(:,  1)= mdl.CoefficientNames';
            r_swglm{t}{r,2}(:,2)=  num2cell(table2array( mdl.Coefficients(:,1) )); 
            r_swglm{t}{r,3}=mdl; 
    ...
    if request.do_stepwiseglm_plot
        request.do_stepwiseglm_plotwhich=[4 5 6 7]; % see details on r_swglm for details of identity
%         f.subplotcols=4;  f.figwidth= 600; f.figheight=500; f.subplot_VerHorz=[0.1 0.05];  f.fig_BotTop=[0.05 0.05];  f.fig_LeftRight=[0.02 0.05];  f.nancol=[0 0 0];
        f.subplotcols=4;  f.figwidth= 1200; f.figheight=1000; f.subplot_VerHorz=[0.03 0.03];  f.fig_BotTop=[0.05 0.05];  f.fig_LeftRight=[0.02 0.05];  f.nancol=[0 0 0];
        ...
        for w=1:length(request.do_stepwiseglm_plotwhich)
            ...
            for r=1:log.n_rois
                ...
                % Plot cF
                subtightplot(log.n_rois , f.subplotcols, k,  f.subplot_VerHorz,  f.fig_BotTop, f.fig_LeftRight);
                imagescnan( r_swglm{1}{r,  request.do_stepwiseglm_plotwhich(w)}, 'NaNColor', f.nancol);  colorbar; set(gca, 'XTick', [])
...
%                     sum(cellfun(@(x)1-isempty(x),  strfind(rt_swglm{1}(1,:), r_swglm{t}{r,2}{p,1}))) ==1
                    ...
                    rt_swglm{t} = [rt_swglm{1} [r_swglm{t}{r,2}{p,1}; repmat({' '}, log.n_rois,1)]];
    ...
    openvar a
    a=repmat({' '}, log.n_rois*2+3, 20); 
    a(1:log.n_rois+1, 1:size(rt_swglm{1},2))= rt_swglm{1}; 
    a(log.n_rois+3:log.n_rois*2+3, 1:size(rt_swglm{2},2))= rt_swglm{2}; 
